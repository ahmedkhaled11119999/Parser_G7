...
def statment():
    ...
    if(len(outputs)):
        newnode = Leaf(outputs[iterator].tokenvalue, currentnode, Parents[-1])
...
def generate_tree():
    ...
    for Node in Nodes:
        if(Node.is_statment()):
            dot.node(str(Node.Node),Node.value,shape='square',color='#8F0000',fontcolor='#03098C')
        else:
            dot.node(str(Node.Node),Node.value,color='#8F0000',fontcolor='#2CAFB2')
    for Node in Nodes:
        if(Node.parentNode!=0)and (Node.connectParent):
            dot.edge(str(Node.parentNode),str(Node.Node),color='#8F0000')
    ...
    for number in range(len(Nodes)):
        for number2 in range(number+1,len(Nodes)):
            ...
            Nodes[number2].is_statment() and (Nodes[number].is_statment())):
                dot.edge(str(Nodes[number].Node),str(Nodes[number2].Node),constraint='false',color='#8F0000')
                break
